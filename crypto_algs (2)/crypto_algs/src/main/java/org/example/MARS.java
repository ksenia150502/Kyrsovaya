...
public class MARS {
    ...
    private static int leftShift(int val, int pas) {

        return (val << pas) | (val >>> (32 - pas));
    }

    private static int rightShift(int val, int pas) {
    ...
    private static int[] expandKey(byte[] key) {
        ...
        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < T.length; i++)
                T[i] = T[i] ^ (leftShift(T[Math.abs(i - 7 % 15)] ^ T[Math.abs(i - 2 % 15)], 3) ^ (4 * i + j));
            for (int c = 0; c < 4; c++)
                for (int i = 0; i < T.length; i++)
                    T[i] = T[i] + leftShift(sBox[(T[Math.abs(i - 1 % 15)] & 0x000001ff)], 9);
            for (int i = 0; i <= 9; i++)
                tmp[10 * j + i] = T[4 * i % 15];
        }
    ...
    public static byte[] encryptBlock(byte[] in) {
        ...
        for (int i = 0; i <= 7; i++) {
            B = B ^ sBox[A & 0xff];
            B = B + sBox[(rightShift(A, 8) & 0xff) + 256];
            C = C + sBox[rightShift(A, 16) & 0xff];
            D = D ^ sBox[(rightShift(A, 24) & 0xff) + 256];
        ...
        for (int i = 0; i <= 7; i++) {
            ...
            B = B ^ sBox[(A & 0xff) + 256];
            C = C - sBox[rightShift(A, 24) & 0xff];
            D = D - sBox[(rightShift(A, 16) & 0xff) + 256];
            D = D ^ sBox[rightShift(A, 8) & 0xff];
    ...
    private static int[] eFunc(int in, int k1, int k2) {
        ...
        int M, L, R;
        M = in + k1;
        R = leftShift(in, 13) * k2;
        L = sBox[M & 0x000001ff];
        R = leftShift(R, 5);
        M = leftShift(M, R & 0x0000001f);
        L = L ^ R;
        R = leftShift(R, 5);
        L = L ^ R;
        L = leftShift(L, R & 0x0000001f);
...
public static byte[] encrypt(byte[] in, byte[] key) {
    ...
    int i;
    len = 16 - in.length % 16;
    padding = new byte[len];
    padding[0] = (byte) 0x80;

    for (i = 1; i < len; i++)
        padding[i] = (byte) len;

    byte[] tmp = new byte[in.length + len];
    byte[] block = new byte[16];

    int count = 0;

    for (i = 0; i < in.length + len; i++) {
        if (i > 0 && i % 16 == 0) {
